...
class SearchConsole::GetFourteenDayAfterTestWorker
  ...
  def perform(shop_id, testing_page_urls, gscDate)
    ...
    if shop && shop.google_website
      ...
      startTime = (gscDate.to_date - 14.days).strftime("%Y-%m-%d")
      ...
      if results.rows != nil
        results.rows.each do |row|
          if testing_page_urls.include? row.keys.first
            SearchConsole::SaveFourteenDayAfterFinalTestDataWorker.perform_async(shop.id, row)
          end
        end
      end
    end
